...
use crate::constants::attr::{SERVICE_TAG, RESOURCE_TAG, ENV_TAG};
...
mod tests {
    use super::*;
    use opentelemetry::trace::{SpanKind, TraceFlags, TraceState, Status};
    ...
    fn create_attributes(service: &'static str, resource: &'static str, env: &'static str) -> Vec<KeyValue> {
        vec![
    ...
    fn test_sampling_rule_creation() {
        let rule = SamplingRule::new(
            ...
            Some(HashMap::from([("custom-tag".to_string(), "tag-value".to_string())])),
    ...
    fn test_sampling_rule_matches() {
        ...
        let rule = SamplingRule::new(
            ...
            Some("http-*".to_string()), 
            None, 
            Some(HashMap::from([("custom_key".to_string(), "custom_value".to_string())])),
            None
        );

        // Should match this span
        let matching_attributes = create_attributes_with_custom_tag(
            "web-service", "some-resource", "custom_key", "custom_value"
        );
        ...
        let wrong_service = create_attributes_with_custom_tag(
            "api-service", "some-resource", "custom_key", "custom_value"
        );
        ...
        let wrong_tag_value = create_attributes_with_custom_tag(
            "web-service", "some-resource", "custom_key", "different_value"
        );
        ...
        let wrong_tag_key = create_attributes_with_custom_tag(
            "web-service", "some-resource", "different_key", "custom_value"
        );
    ...
    fn test_service_key_generation() {
        ...
        assert_eq!(sampler.service_key(attrs.as_slice()), "service:test-service,env:production");
        ...
        assert_eq!(sampler.service_key(attrs_no_env.as_slice()), "service:test-service,env:");
    }
    ...
    fn test_update_service_rates() {
        ...
        // Verify keys exist
        assert!(sampler.service_samplers.contains_key("service:web,env:prod"));
        assert!(sampler.service_samplers.contains_key("service:api,env:prod"));
    ...
    fn test_get_sampling_mechanism() {
        ...
        let rule_customer = SamplingRule::new(0.1, None, None, None, None, Some("customer".to_string()));
        let rule_dynamic = SamplingRule::new(0.2, None, None, None, None, Some("dynamic".to_string()));
        let rule_default = SamplingRule::new(0.3, None, None, None, None, Some("default".to_string()));
        ...
        assert_eq!(mechanism2, SamplingMechanism::RemoteDynamicTraceSamplingRule);
    ...
    fn test_add_dd_sampling_tags() {
        ...
        let rate_limit = 100;
        let attrs_with_limit = sampler.add_dd_sampling_tags(&decision, mechanism, sample_rate, Some(rate_limit));
        ...
        let agent_attrs = sampler.add_dd_sampling_tags(&decision, agent_mechanism, agent_rate, None);
        ...
        for attr in &agent_attrs {
            assert_ne!(attr.key.as_str(), SAMPLING_RULE_RATE_TAG_KEY, 
