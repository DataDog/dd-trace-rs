...
pub(crate) async fn start_invocation<E: Serialize>(
    request_id: &str,
    event: &E,
) -> Option<crate::InvocationContext> {
...
fn parse_extension_response(headers: &reqwest::header::HeaderMap) -> Option<ExtensionTrace> {
    ...
    let flags = if sampling_priority > 0 {
        TraceFlags::SAMPLED
    } else {
        TraceFlags::default()
    };
    ...
    dd_lambda_debug!(
        "parsed trace context: trace_id={trace_id} span_id={span_id} parent_id={parent_id} sampling={sampling_priority}"
    );
