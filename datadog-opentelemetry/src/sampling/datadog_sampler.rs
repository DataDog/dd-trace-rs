...
impl TraceRootSamplingInfo {
    ...
    pub fn rl_effective_rate(&self) -> Option<i32> {
...
impl DdSamplingResult {
        ...
        F: crate::sampling::AttributeFactory,
    {
        ...
        if let Some(limit) = root_info.rl_effective_rate() {
            ...
            result.push(factory.create_i64(RL_EFFECTIVE_RATE, limit as i64));
        } else {
...
mod tests {
    ...
    use super::{HTTP_RESPONSE_STATUS_CODE, HTTP_STATUS_CODE};
    ...
    impl<'a> SpanProperties for TestSpan<'a> {
        ...
        type AttributesIter<'b>
            = std::slice::Iter<'b, TestAttribute>
        where
            Self: 'b;
    ...
    fn test_add_dd_sampling_tags() {
        ...
        for attr in &agent_attrs {
            if attr.key() == SAMPLING_AGENT_RATE_TAG_KEY {
                let value_float = match attr.value() {
                    TestValue::F64(f) => *f,
