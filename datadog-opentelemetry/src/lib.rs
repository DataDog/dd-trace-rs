...
fn make_tracer(
    ...
    if config.remote_config_enabled() {
        ...
        if let Some(sampler_callback) = sampler_callback {
            ...
            mutable_config.lock().unwrap().add_remote_config_callback("datadog_sampler_on_rules_update".to_string(), move |rules| {
                // Convert rules to JSON string for the sampler
                if let Ok(json_str) = serde_json::to_string(&rules) {
                    sampler_callback_clone(&json_str);
                    dd_trace::dd_info!("RemoteConfigClient: Updated sampler with new rules");
                }
            });
        }
