...
use super::{
    super::sdk_span::SdkSpan,
    super::transform::{CowStr, DdSpanEvent, DdSpanLink},
    DdSpan,
};
...
mod tests {
    ...
    fn hashmap_diff<'a, V: PartialEq + Debug>(
        ...
        let mut a = output.iter().map(|(k, v)| (k, v)).collect::<Vec<_>>();
        let mut b = expected.iter().map(|(k, v)| (k, v)).collect::<Vec<_>>();
