...
use libdd_trace_utils::span::{v04::VecMap, SpanText};
use opentelemetry::{
    trace::{Link, SpanKind},
    Key, KeyValue, Value,
};
...
pub fn otel_span_to_dd_span<'a>(
    ...
    if !dd_span
        .meta
        .contains_key(&SpanStr::from_static_str("version"))
    {
        ...
        if !version.is_empty() {
            dd_span.meta.insert(
                SpanStr::from_static_str("version"),
                SpanStr::from_string(version),
            );
        }
    }
    ...
    if !dd_span.meta.contains_key(&SpanStr::from_static_str("env")) {
        let env = get_otel_env(&span_extracted);
        if !env.is_empty() {
            dd_span
                .meta
                .insert(SpanStr::from_static_str("env"), SpanStr::from_cow(env));
        }
    }
