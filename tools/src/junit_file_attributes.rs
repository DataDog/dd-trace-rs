...
/// (e.g., a package `my-crate` with lib target `my_crate`).
///
/// Nextest generates different classname formats:
/// - Integration tests: `classname="package::target_name"` (e.g., `my-crate::test_send_data`)
/// - Unit tests: `classname="package"` (e.g., `my-crate`)
///
...
fn resolve_module_file(test_name: &str, src_dir: &Path, workspace_root: &Path) -> Option<String> {
    ...
    // Find the "tests" or "test" module and take everything before it
    let module_parts: Vec<&str> = parts
        .iter()
        .take_while(|&&p| p != "tests" && p != "test")
        .copied()
        .collect();
...
mod tests {
    ...
    fn test_target_lookup_lib_alias() {
        ...
        lookup.insert_lib(
            "my-profiling-ffi",
            "my_profiling_ffi_target",
            PathBuf::from("/src/lib.rs"),
        );

        // Should find via alias
        let result = lookup.get("my-profiling-ffi", None);
    ...
    fn test_target_lookup_test_target() {
        ...
        lookup.insert_target(
            "my-crate",
            "test_something",
            PathBuf::from("/tests/test_something.rs"),
        );

        let result = lookup.get("my-crate", Some("test_something"));
        assert!(result.is_some());
        assert_eq!(result.unwrap(), &PathBuf::from("/tests/test_something.rs"));
    }
...
<testsuites>
  <testsuite name="my-crate">
    ...
    #[test]
    fn test_process_junit_xml_adds_file_attribute_test_module() {
        let mut lookup = TargetLookup::new();
        lookup.insert_lib(
            "my-crate",
            "my-crate",
...
<testsuites>
  <testsuite name="my-crate">
    <testcase classname="my-crate" name="test::test_foo" />
  </testsuite>
</testsuites>"#;
